...
def metric_cluster_main(query, solr_results):
    # query = query[8:]
    stop_words = set(stopwords.words(['english','spanish','italian']))
    tokens = []
    token_counts = {}
    tokens_map = {}
    document_ids = []
    ...
    print('Completed making stem map')
    metric_clusters = get_metric_clusters(tokens_map, stem_map, query)
    print('Completed getting metric clusters')
    metric_clusters2 = [elem for cluster in metric_clusters for elem in cluster]
    print('Completed flattening metric clusters')
    # print(metric_clusters2[:10])
    metric_clusters2.sort(key=lambda x:x.value,reverse=True)
    
    porter_stemmer = PorterStemmer()
    metric_words = [element.v for element in metric_clusters2]
    
    metric_word_stems = [porter_stemmer.stem(word) for word in metric_words]
    metric_word_stems = list(set(metric_word_stems))
    i = 0
    
    query_stems = [porter_stemmer.stem(word) for word in query.split(' ')]
    query_stems = list(set(query_stems))
    while(i<min(2, len(metric_words))):
...
# query = "content:opera"
# solr_results = get_results_from_solr(query, 5)
# print(metric_cluster_main(query,solr_results))    